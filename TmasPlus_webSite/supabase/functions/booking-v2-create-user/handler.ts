...
export function createOnboardingHandler(createClient: any, env: (name: string) => string | undefined) {
  return async (req: Request): Promise<Response> => {
    ...
    try {
      ...
      if (url !== 'https://lhqhdnjmewyipuwifzsl.supabase.co') return json({ error: 'Solo disponible en Prueba' }, 403);
      ...
      const db = viewer.schema('booking_v2');
      const { data: allowed, error: permissionError } = await db.rpc('is_admin');
      if (permissionError || allowed !== true) return json({ error: 'Administrador autorizado requerido' }, 403);
      let input;
      ...
      const { data, error } = await db.rpc('admin_create_profile', {
