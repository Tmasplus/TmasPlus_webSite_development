...
// Cancela una reserva en la BD secundaria (App) con SERVICE ROLE.
//
// Misma razón que create-booking / set-user-blocked: el dashboard se autentica
...
// primario y actualizamos con service role (bypassa RLS).
...
serve(async (req: Request) => {
  ...
  // 3. Construir los campos de cancelación en el servidor (no se confía al cliente).
  // cancelled_at -> bigint (epoch ms) ; cancellation_time -> time (HH:MM:SS)
  const now = new Date();
  const updateFields = {
    status: "CANCELLED",
    cancelled_at: now.getTime(),
    cancellation_time: now.toISOString().slice(11, 19),
    reason: body.reason || "Cancelada por administrador",
    cancelled_by: "admin",
  };

  // 4. Actualizar en el proyecto secundario con service role (ignora RLS)
  ...
  if (updateErr) {
    return json({ error: `Error al cancelar la reserva: ${updateErr.message}` }, 500);
  }
  if (!updated) {
